...
public class DataRacePluginTests(ITestOutputHelper testOutput)
{
    ...
    [Theory]
    [MemberData(nameof(SdkVersions.AllWithFastTrackOnly), MemberType = typeof(SdkVersions))]
    public Task NoDataRace_ConcurrentDictionaryLockFreeReaderObservesPublication(string sdk, string plugin)
        => AssertDoesNotDetectDataRace("Test_NoDataRace_ConcurrentDictionaryLockFreeReaderObservesPublication", sdk, plugin);

    [Theory]
    [MemberData(nameof(SdkVersions.AllWithFastTrackOnly), MemberType = typeof(SdkVersions))]
    public Task NoDataRace_LazyLockFreeReaderObservesPublication(string sdk, string plugin)
        => AssertDoesNotDetectDataRace("Test_NoDataRace_LazyLockFreeReaderObservesPublication", sdk, plugin);

    [Theory]
    [MemberData(nameof(SdkVersions.AllWithFastTrackOnly), MemberType = typeof(SdkVersions))]
    public Task CanDetectDataRace_WriteValueFromGenericMethodCall(string sdk, string plugin)
        => AssertDetectsDataRace("Test_DataRace_WriteValueFromGenericMethodCall", sdk, plugin);
    ...
    [Theory]
    [MemberData(nameof(SdkVersions.AllWithFastTrackOnly), MemberType = typeof(SdkVersions))]
    public Task NoDataRace_InterlockedCounter_Instance_ConcurrentIncrement(string sdk, string plugin)
        => AssertDoesNotDetectDataRace("Test_NoDataRace_InterlockedCounter_Instance_ConcurrentIncrement", sdk, plugin);

    [Theory]
    [MemberData(nameof(SdkVersions.AllWithFastTrackOnly), MemberType = typeof(SdkVersions))]
    public Task CanDetectDataRace_InterlockedFlag_Instance_WriteAfterPublish(string sdk, string plugin)
        => AssertDetectsDataRace("Test_DataRace_InterlockedFlag_Instance_WriteAfterPublish", sdk, plugin);
