...
namespace SharpDetect.E2ETests.Subject
{
    public static partial class Program
    {
        ...
        {
            DataRace.Test_ThreadStatic_ReferenceType = new object();
            RunConcurrently(
                () => DataRace.Test_ThreadStatic_ReferenceType = new object(),
                () => DataRace.Test_ThreadStatic_ReferenceType = new object());
        }
        ...
        public static void Test_NoDataRace_ThreadStatic_ValueType()
        {
            ...
            RunConcurrently(
                () => DataRace.Test_ThreadStatic_ValueType = 123,
                () => DataRace.Test_ThreadStatic_ValueType = 321);
        }
        ...
        public static void Test_NoDataRace_ThreadStatic_ReadWrite()
        {
            DataRace.Test_ThreadStatic_ValueType = 0;
            RunConcurrently(
                () => _ = DataRace.Test_ThreadStatic_ValueType,
                () => DataRace.Test_ThreadStatic_ValueType = 123);
        }
        ...
        public static void Test_NoDataRace_VolatileField_Static_ReadWriteNoRace()
        {
            ...
            RunConcurrently(
                () => _ = DataRace.Test_Volatile_ValueType_Static,
                () => DataRace.Test_Volatile_ValueType_Static = 123);
        }
        ...
        public static void Test_NoDataRace_VolatileField_Instance_ReadWriteNoRace()
        {
            ...
            RunConcurrently(
                () => _ = dataRace.Test_Volatile_ValueType_Instance,
                () => dataRace.Test_Volatile_ValueType_Instance = 123);
        }
        ...
        public static void Test_NoDataRace_VolatileExplicitAccess_Static_ReadWriteNoRace()
        {
            DataRace.Test_Volatile_ValueType_Static = 0;
            RunConcurrently(
                () => Volatile.Read(ref DataRace.Test_DataRace_ValueType_Static),
                () => Volatile.Write(ref DataRace.Test_DataRace_ValueType_Static, 123));
        }
        ...
        {
            var dictionary = new ConcurrentDictionary<int, DataRace>();
            var published = new DataRace();
            RunConcurrently(
                () =>
                {
                    published.Test_DataRace_ValueType_InstanceField = 42;
                    dictionary.TryAdd(1, published);
                },
                () =>
                {
                    DataRace? observed = null;
                    for (var i = 0; i < PublicationSpinLimit && observed is null; i++)
                    {
                        if (!dictionary.TryGetValue(1, out observed))
                            Thread.Sleep(1);
                    }

                    if (observed is null)
                        throw new TimeoutException("Publisher never made the value visible to the lock-free reader.");

                    _ = observed.Test_DataRace_ValueType_InstanceField;
                });
        }

        public static void Test_NoDataRace_LazyLockFreeReaderObservesPublication()
        {
            var published = new DataRace();
            var lazy = new Lazy<DataRace>(() => published);
            RunConcurrently(
                () =>
                {
                    published.Test_DataRace_ValueType_InstanceField = 42;
                ...
                () =>
                {
                    for (var i = 0; i < PublicationSpinLimit && !lazy.IsValueCreated; i++)
                        Thread.Sleep(1);

                    if (!lazy.IsValueCreated)
                        throw new TimeoutException("Publisher never materialized the lazy value.");

                    var observed = lazy.Value;
                    _ = observed.Test_DataRace_ValueType_InstanceField;
                });
        }
        ...
        public static void Test_ConcurrentDictionaryMethods_Store()
        {
            ...
            // Each of these publishes a value through a distinct store-shaped hook
            dictionary.TryAdd(1, new object());
            dictionary[2] = new object();
            dictionary.TryUpdate(2, new object(), dictionary[2]);
        }

        public static void Test_ConcurrentDictionaryMethods_LoadByRef()
        {
            var dictionary = new ConcurrentDictionary<int, object>();
            dictionary[1] = new object();
            // Both observe the value through an out parameter rather than a return value
            dictionary.TryGetValue(1, out _);
            dictionary.TryRemove(1, out _);
        }

        public static void Test_ConcurrentDictionaryMethods_AddOrUpdate()
        {
            var dictionary = new ConcurrentDictionary<int, object>();
            _ = dictionary.AddOrUpdate(1, new object(), (_, existing) => existing);
            _ = dictionary.AddOrUpdate(1, _ => new object(), (_, existing) => existing);
        }
        ...
        public static void Test_MultiProcess_ChildExitsBeforeParent()
        {
            ...
            lock (new object()) { }
            if (!child.WaitForExit(timeout: WaitTimeout))
                throw new TimeoutException("Timed out while waiting for child process to exit.");
        }
        ...
        private static void RunConcurrently(params Action[] actions)
        {
            ...
            var exceptions = new Exception?[actions.Length];

            var threads = actions
                .Select((action, i) => new Thread(() =>
                {
                    var workerId = i + 1;
                    Thread.CurrentThread.Name = $"TEST_Thread{workerId}";
                    ready.Signal();
                    if (!start.Wait(WaitTimeout))
                    {
                        exceptions[i] = new TimeoutException($"Timed out while waiting to release worker thread {workerId}.");
                        return;
                    }
            ...
            if (!ready.Wait(WaitTimeout))
                throw new TimeoutException("Timed out while waiting for worker threads to become ready.");

            start.Set();

            foreach (var t in threads)
            {
                if (!t.Join(WaitTimeout))
                    throw new TimeoutException($"Timed out while waiting for thread '{t.Name}' to complete.");
            }
            
            var failures = exceptions.Where(e => e is not null)
                .Select(e => e!)
                .ToArray();
            
            if (failures.Length > 0)
                throw new AggregateException("One or more worker threads threw an exception.", failures);
        }
    }
}
