...
namespace SharpDetect.TestUtils.E2E
{
    ...
    public class ReporterPlugin : NopPlugin
    {
        ...
        public override void AnalysisStarted(EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(AnalysisStarted), string.Empty, null, default));
        public override void AnalysisEnded(EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(AnalysisEnded), string.Empty, null, default));
        public override void MethodCalled(FunctionInfo method, IArgumentsList? arguments, EventInfo info)
        {
            ...
            reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(MethodCalled), methodDef?.FullName ?? "<unknown-method>", null, default));
        }
        ...
        {
            metadataContext.GetResolver(info.Runtime.ProcessId).TryGetMethodDef(method, new(method.ModuleId), resolveWrappers: true, out var methodDef);
            reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(MethodReturned), methodDef?.FullName ?? "<unknown-method>", null, default));
        }
        public override void LockAcquireAttempted(IShadowObject instance, EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(LockAcquireAttempted), string.Empty, new[] { instance }, default));
        public override void LockAcquireReturned(IShadowObject instance, bool isSuccess, EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(LockAcquireReturned), string.Empty, new[] { instance }, default));
        public override void LockReleased(IShadowObject instance, EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(LockReleased), string.Empty, new[] { instance }, default));
        ...
        public override void GarbageCollectionStarted(EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(GarbageCollectionStarted), string.Empty, null, default));
        public override void GarbageCollectionFinished(EventInfo info) => reportingService.Report(new InformationReport(nameof(ReporterPlugin), nameof(GarbageCollectionFinished), string.Empty, null, default));
    }
}
