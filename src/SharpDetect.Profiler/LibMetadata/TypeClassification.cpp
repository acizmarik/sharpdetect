...
LibProfiler::GenericTypeArgKind LibProfiler::ClassifyMethodGenericArgument(
	ICorProfilerInfo2& corProfilerInfo,
	const ModuleDefResolver& resolveModuleDef,
	const FunctionID functionId,
	const COR_PRF_FRAME_INFO frameInfo,
...
{
	ULONG32 numMethodTypeArgs = 0;
	if (FAILED(corProfilerInfo.GetFunctionInfo2(functionId, frameInfo, nullptr, nullptr, nullptr, 0, &numMethodTypeArgs, nullptr)) ||
		typeArgIndex >= numMethodTypeArgs)
		return GenericTypeArgKind::Unknown;

	std::vector<ClassID> methodTypeArgs(numMethodTypeArgs);
	if (FAILED(corProfilerInfo.GetFunctionInfo2(functionId, frameInfo, nullptr, nullptr, nullptr, numMethodTypeArgs, &numMethodTypeArgs, methodTypeArgs.data())))
		return GenericTypeArgKind::Unknown;

	return ClassifyClass(corProfilerInfo, resolveModuleDef, methodTypeArgs[typeArgIndex]);
}
