...
namespace SharpDetect.Core.Utilities
{
    internal static class MethodsGenerator
    {
        ...
        {
            var helper = new MethodDefUser(Enum.GetName(type), GetMethodSig(type, corLibTypes), MethodImplAttributes.IL | MethodImplAttributes.Managed,
                MethodAttributes.Static | MethodAttributes.Public);
            // Note: there is no need to implement this here, the implementation is straight-forward and supplied by profiler
            return helper;
        }

        private static MethodSig GetMethodSig(MethodType type, ICorLibTypes corLibTypes)
        {
            return type switch
            {
